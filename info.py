...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002061859450').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1865038982').split()]
...
auth_channel = environ.get('AUTH_CHANNEL', '-1002426527624') # give your force subscribe channel id here else leave it blank
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://PmShortnerBot:PmShortnerBot@cluster0.7rkil0p.mongodb.net/?retryWrites=true&w=majority") # IF Multiple Database Is False Then Fill Only This Database Url.
...
if 'DYNO' in environ:
    ...
    ON_HEROKU = False
URL = environ.get("URL", "dominant-marji-sbplays10.koyeb.app/")
